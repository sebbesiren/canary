...
void ProtocolGame::sendTextMessage(const TextMessage &message) {
	if (message.type == MESSAGE_NONE) {
		g_logger().error("[ProtocolGame::sendTextMessage] - Message type is wrong, missing or invalid for player with name {}, on position {} for message {}", player->getName(), player->getPosition().toString(), message.text);
...
void ProtocolGame::parseWheelGemAction(NetworkMessage &msg) {
	...
	addGameTask(&Game::playerWheelGemAction, player->getID(), msg);
}
