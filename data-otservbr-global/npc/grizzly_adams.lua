...
local function getCustomTasksText()
	local raceNames = {}
	for _, task in ipairs(tasks.GrizzlyAdams) do
		if task.custom then
	...
	elseif table.contains({ "tasks", "task", "mission" }, message:lower()) then
			...
			for i = 1, #started do
				...
				if getKillCount(player, id) >= tasks.GrizzlyAdams[id].killsRequired then
					...
					for j = 1, #tasks.GrizzlyAdams[id].rewards do
						...
						elseif reward.type:lower() == "bosspoints" and not deny then
							player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.BossPoints, player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.BossPoints) + reward.value[1])
		...
		elseif player:getLevel() >= 130 and player:getStorageValue(POINTSSTORAGE) < 100 then
			npcHandler:say({
				...
				"Custom tasks: " .. getCustomTasksText(),
			}, npc, creature)
		else
			npcHandler:say({
